class ImageConstant {
  // Image folder path
  static const String imagePath = 'assets/images';

  // Blog details images
  static String imgUnsplashJe4ihjeKni =
      '$imagePath/img_unsplash_je4ihje_kni.png';

  // Welcome images
  static String imgImage1 = '$imagePath/img_image_1.png';

  // WelcomeOne images
  static String imgUpcycleRafiki = '$imagePath/img_upcycle_rafiki.svg';

  // WelcomeTwo images
  static String imgHandSewingPana = '$imagePath/img_hand_sewing_pana.svg';

  // WelcomeThree images
  static String imgBrandLoyaltyRafiki =
      '$imagePath/img_brand_loyalty_rafiki.svg';

  // Sign up images
  static String imgEllipse1 = '$imagePath/img_ellipse_1.png';

  // Sign in images
  static String imgEllipse1114x146 = '$imagePath/img_ellipse_1_114x146.png';

  // Complete profile images
  static String imgGroup10 = '$imagePath/img_group_10.svg';

  static String imgGroup11 = '$imagePath/img_group_11.svg';

  // Home images
  static String imgUnsplashGgtwjdt6dci =
      '$imagePath/img_unsplash_ggtwjdt6dci.png';

  static String imgUnsplashGgtwjdt6dci134x170 =
      '$imagePath/img_unsplash_ggtwjdt6dci_134x170.png';

  static String imgUnsplash3125km62vz4 =
      '$imagePath/img_unsplash_3125km62vz4.png';

  static String imgUnsplashGgtwjdt6dci1 =
      '$imagePath/img_unsplash_ggtwjdt6dci_1.png';

  static String imgUnsplashGgtwjdt6dci2 =
      '$imagePath/img_unsplash_ggtwjdt6dci_2.png';

  static String imgUnsplash3125km62vz4134x170 =
      '$imagePath/img_unsplash_3125km62vz4_134x170.png';

  static String imgUnsplashMuf7ly04soi =
      '$imagePath/img_unsplash_muf7ly04soi.png';

  static String imgUnsplashGgtwjdt6dci3 =
      '$imagePath/img_unsplash_ggtwjdt6dci_3.png';

  static String imgUnsplashGgtwjdt6dci4 =
      '$imagePath/img_unsplash_ggtwjdt6dci_4.png';

  static String imgUnsplashGgtwjdt6dci5 =
      '$imagePath/img_unsplash_ggtwjdt6dci_5.png';

  static String imgUnsplashGgtwjdt6dci6 =
      '$imagePath/img_unsplash_ggtwjdt6dci_6.png';

  // Blogs images
  static String imgEdit = '$imagePath/img_edit.svg';

  static String imgUnsplashX8zstuks2pm =
      '$imagePath/img_unsplash_x8zstuks2pm.png';

  // Shop images
  static String imgCart = '$imagePath/img_cart.svg';

  static String imgPlus = '$imagePath/img_plus.svg';

  // Product details Two images
  static const String imgUnsplashNovnxxmdni0 =
      '$imagePath/img_unsplash_novnxxmdni0.png';

  static String imgArrowRightOnprimary =
      '$imagePath/img_arrow_right_onprimary.svg';

  static const String imgUnsplash76wEdo1u1e =
      '$imagePath/img_unsplash_76w_edo1u1e.png';

  static String imgSettingsErrorcontainer =
      '$imagePath/img_settings_errorcontainer.svg';

  static const String imgUnsplashMhuk4se7pey =
      '$imagePath/img_unsplash_mhuk4se7pey.png';

  static String imgUnsplashFur3lnii2jc =
      '$imagePath/img_unsplash_fur3lnii2jc.png';

  static String imgUnsplashEjtjetc8tps =
      '$imagePath/img_unsplash_ejtjetc8tps.png';

  static String imgGrid = '$imagePath/img_grid.svg';

  // Cart images
  static String imgThumbsUpOnprimarycontainer =
      '$imagePath/img_thumbs_up_onprimarycontainer.svg';

  // Profile images
  static String imgUnsplashZhvm3xiohoe =
      '$imagePath/img_unsplash_zhvm3xiohoe.png';

  static String imgUser = '$imagePath/img_user.svg';

  static String imgInfo = '$imagePath/img_info.svg';

  static String imgLockPrimary = '$imagePath/img_lock_primary.svg';

  static String imgInfoPrimary = '$imagePath/img_info_primary.svg';

  static String imgInbox = '$imagePath/img_inbox.svg';

  // Your Profile images
  static String imgUnsplashZhvm3xiohoe142x142 =
      '$imagePath/img_unsplash_zhvm3xiohoe_142x142.png';

  // Settings images
  static String imgContrast = '$imagePath/img_contrast.svg';

  static String imgThumbsUpPrimary = '$imagePath/img_thumbs_up_primary.svg';

  // Common images
  static String imgSettings = '$imagePath/img_settings.svg';

  static String imgBookmark = '$imagePath/img_bookmark.svg';

  static String imgArrowLeft = '$imagePath/img_arrow_left.svg';

  static String imgArrowRight = '$imagePath/img_arrow_right.svg';

  static String imgFrame10 = '$imagePath/img_frame_10.svg';

  static String imgEye = '$imagePath/img_eye.svg';

  static String imgCheckmark = '$imagePath/img_checkmark.svg';

  static String imgGoogle = '$imagePath/img_google.svg';

  static String imgFacebook = '$imagePath/img_facebook.svg';

  static String imgArrowdown = '$imagePath/img_arrowdown.svg';

  static String imgCalendar = '$imagePath/img_calendar.svg';

  static String imgHome = '$imagePath/img_home.svg';

  static String imgForward = '$imagePath/img_forward.svg';

  static String imgTelevision = '$imagePath/img_television.svg';

  static String imgThumbsUp = '$imagePath/img_thumbs_up.svg';

  static String imgLock = '$imagePath/img_lock.svg';

  static String imgSearch = '$imagePath/img_search.svg';

  static String imgTune = '$imagePath/img_tune.svg';

  static String imgUnsplash9okgevjitkk =
      '$imagePath/img_unsplash_9okgevjitkk.png';

  static String imgPlayCircle = '$imagePath/img_play_circle.svg';

  static String imgCalendarOnerror = '$imagePath/img_calendar_onerror.svg';

  static const String imgUnsplashN7wmtrqv5am =
      '$imagePath/img_unsplash_n7wmtrqv5am.png';

  static String imgCartOnprimarycontainer =
      '$imagePath/img_cart_onprimarycontainer.svg';

  static String imgEditOnprimarycontainer =
      '$imagePath/img_edit_onprimarycontainer.svg';

  static String imgArrowRightPrimary = '$imagePath/img_arrow_right_primary.svg';

  static String imageNotFound = 'assets/images/image_not_found.png';

  static String avatarPlaceholder = '$imagePath/avatar_placeholder.jpg';

  static const String sofa = '../../../assets/images/maxresdefault.jpg';

  static const String clothes = '../../../assets/images/quanao.jpg';

  static const String bottle = '../../../assets/images/binh_nuoc.png';
}
